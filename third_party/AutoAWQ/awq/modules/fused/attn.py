...
class QuantAttentionFused(nn.Module):
    ...
    def forward(
        ...
        if seqlen > 1 or self.partial_rotary_factor < 1 or not FT_INSTALLED:
            ...
            # SDPA does not expose logits, so models such as Gemma2 that require
            # attention-logit softcapping must use the explicit attention path.
            if self.use_sdpa and self.attn_logit_softcapping is None:
