...
class Qwen2MoeConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Allow per-layer num_experts list while keeping a scalar for external checks (some code expects int).
        if isinstance(num_experts, list):
            self.num_experts_list = num_experts
            self.num_experts = max(num_experts) if len(num_experts) > 0 else 0
        else:
            self.num_experts_list = None
