...
if is_flash_attn_2_available():
    # Keep this relative so the bundled legacy snapshot remains self-contained.
...
class Qwen2MoeFlashAttention2(Qwen2MoeAttention):
    ...
    def forward(
        ...
        # flash-attn path does not return attn_weights; keep None by default
        attn_weights = None
        ...
        # attn_weights stays None; output_attentions not supported in this path
...
class Qwen2MoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if apply_mask:
            ...
            masked_ids = torch.nonzero(self.router_logits_mask == float("-inf"), as_tuple=False).view(-1).tolist()
...
class Qwen2MoeDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        position_embeddings: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,  # kept for BC with patched attention
        **kwargs,
        ...
        if self.post_attention_layernorm is None and self.mlp is None:
            ...
            if isinstance(mlp_output, tuple):
                if len(mlp_output) >= 2:
                    # Qwen2MoeSparseMoeBlock returns (hidden_states, router_logits, routing_entropy)
                    hidden_states, router_logits = mlp_output[0], mlp_output[1]
                else:
