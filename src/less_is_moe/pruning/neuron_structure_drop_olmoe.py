"""
Structural neuron-level drop for OLMoE-1B-7B.
...
Strategy — structural removal (shapes shrink):
    ...
    OLMoE has no shared_expert; only routed experts are modified.

Equivalence to zero-masking (neuron_drop_olmoe.py):
    OLMoE expert projections all have `bias=False`, so removing the j-th row
    of gate/up and the j-th col of down is bit-equivalent to zero-masking
    them (modulo FP rounding).

Importance metric (same as neuron_drop_olmoe.py):
...
Usage:
    scripts/prune/neuron_structure_drop_olmoe.sh \\
...
# ---------------------------------------------------------------------------
# Reuse calibration loaders, MoE utilities, and scoring from neuron_drop_olmoe
# ---------------------------------------------------------------------------

from .neuron_drop_olmoe import (
...
# ---------------------------------------------------------------------------
# Detect pre-existing zero-masked neurons (from neuron_drop_olmoe.py)
# ---------------------------------------------------------------------------

@torch.no_grad()
def find_zeroed_neurons(model, moe_layer_indices):
    """Scan each routed expert for fully-zero gate_proj rows.

    Returns dict {layer_idx: {expert_idx: sorted list of zeroed-neuron indices}}.
    """
    drop_per_layer = {}
...
def structurally_remove_neurons(model, drop_per_layer, moe_layer_indices):
    """Replace each routed expert's gate/up/down with smaller Linears.

    All experts must drop the same count per layer so that the resulting
    d_ffn is uniform (otherwise OLMoE's tensor stack assumption breaks).
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Structural neuron-level drop for OLMoE"
    )
    parser.add_argument("--model_name_or_path", type=str, required=True)
    parser.add_argument("--output_dir", type=str, required=True)
    parser.add_argument("--drop_ratio", type=float, default=None,
                        help="Fraction of neurons to remove per expert. "
                             "Not needed when --from_zeroed_model is set.")
    parser.add_argument("--from_zeroed_model", action="store_true",
                        help="Detect already-zeroed neurons in `--model_name_or_path` "
                             "(a neuron_drop output) and shrink them structurally.")
    ...
    # OLMoE config uses `intermediate_size` for the per-expert FFN dim.
    old_int_size = getattr(model.config, "intermediate_size", None)
    model.config.intermediate_size = new_d_ffn
    print(f"config.intermediate_size: {old_int_size} -> {new_d_ffn}")
