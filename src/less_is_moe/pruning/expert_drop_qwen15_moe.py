"""
Standalone expert-drop script for Qwen1.5-MoE-A2.7B (also works for other Qwen2-MoE variants).

Usage:
    # With local JSON calibration data:
    python -m less_is_moe.pruning.expert_drop_qwen15_moe \
    ...
    python -m less_is_moe.pruning.expert_drop_qwen15_moe \
      --model_name_or_path Qwen/Qwen1.5-MoE-A2.7B \
      --output_dir outputs/qwen15-pruned \
      --method bias_pruning \
      --n_samples 128 \
      --seq_len 2048 \
      --dataset_name RoxanneWsyw/gsm \
      ...
      --bias_update_speed 4e-2 \
      --bias_criterion median \
      --bias_clip 10.0 \
      --bias_epochs 2 \
      --bias_threshold 0.0

Qwen1.5-MoE differences from Qwen3-MoE:
  - Has shared_expert + shared_expert_gate (not pruned, only routed experts are pruned)
  - Uses Qwen2MoeSparseMoeBlock
  - config.num_experts (int=60), config.num_experts_per_tok (int=4)
  - No position_embeddings in decoder layer forward (all args optional)
"""
...
# Preset -> (candidate dataset repos tried in order, default split, formatter).
# Multiple repos are tried because these benchmarks have several mirrors on the
# Hub and availability changes over time.
_PRESET_REGISTRY = {
    "ceval": {
...
def get_moe_layer_info(model):
    """
    Return (moe_layer_indices, num_experts_per_layer) for Qwen1.5-MoE / Qwen2-MoE.
    """
    config = model.config
...
class RouterScoreCollector:
    ...
    def hook_fn(self, module, input, output):
        ...
        router_logits = output[1]  # (batch*seq, num_experts)
        router_logits = router_logits.reshape(-1, router_logits.shape[-1])
        routing_weights = F.softmax(router_logits, dim=1, dtype=torch.float)

        batch_score = routing_weights.sum(0)  # (num_experts,)
...
def collect_routing_scores(model, calib_batches, moe_layer_indices):
    """
    Run calibration data through the model layer-by-layer, collecting routing scores.
    Returns dict: {layer_idx: avg_score_tensor}.
    """
    ...
    # Layer-by-layer forward with hooks on MoE blocks
    scores_per_layer = {}
...
class BiasCollector:
    ...
    def hook_fn(self, module, input, output):
        ...
        # Add current bias to logits (same as SFT: logits = logits + self.bias)
        ...
        expert_usage = routing_weights.sum(dim=0)  # (num_experts,)

        # Bias update (mirrors sft_aux_new_bias_tracker.py logic)
        with torch.no_grad():
...
def collect_bias_scores(model, calib_batches, moe_layer_indices,
    ...
    """
    Simulate aux-free bias accumulation over calibration data.
    Returns dict: {layer_idx: final_bias_tensor}.
    """
    device = next(model.parameters()).device
    layers = model.model.layers
    ...
    # Initialize bias collectors per MoE layer
    collectors = {}
...
def collect_weight_magnitude_scores(model, moe_layer_indices):
    """
    Compute expert importance based on weight magnitude (data-free).

    For each expert, the score is the L2 norm of all its parameters
    (gate_proj, up_proj, down_proj weights concatenated).
    Experts with larger weight norms are considered more important.
    """
    layers = model.model.layers
    scores_per_layer = {}
    ...
    for layer_idx in tqdm(moe_layer_indices, desc="Collecting weight magnitude scores"):
        ...
        for eid, expert in enumerate(moe_block.experts):
            # Concatenate all expert parameters and compute L2 norm
...
# ---------------------------------------------------------------------------
# Gradient-based score collection
# ---------------------------------------------------------------------------

def collect_gradient_scores(model, calib_batches, moe_layer_indices):
    """
    Compute gradient-based expert importance scores.

    For each MoE layer's gate, computes Taylor first-order importance per expert:
        importance_j = || gate.weight[j] * ∂L/∂gate.weight[j] ||

    This measures how much the loss would change if expert j's routing weight
    were zeroed out (first-order Taylor approximation).

    Uses standard LM loss (labels = input_ids) on calibration data.
    Processes one sample at a time and accumulates to limit memory usage.
    """
    device = next(model.parameters()).device

    # Freeze everything except gate weights (saves memory on non-gate grads)
    ...
    model.train()
    for batch_idx, batch in enumerate(tqdm(calib_batches, desc="Collecting gradient scores")):
        ...
        for layer_idx, gate_w in gate_params.items():
            if gate_w.grad is None:
                continue
            # Taylor importance: |w * grad| per expert row, then L2 norm
            taylor = (gate_w.data.float() * gate_w.grad.float()).abs()
            importance = taylor.norm(dim=1).detach().cpu()  # (num_experts,)
...
def collect_pure_gradient_scores(model, calib_batches, moe_layer_indices):
    """
    Compute expert importance based on the average absolute gradient of the
    router gate weight matrix: mean(|∂L/∂W_gate|) per expert row.

    Unlike ``collect_gradient_scores`` which uses the Taylor approximation
    |W * ∂L/∂W|, this measures pure gradient magnitude — how sensitive the
    loss is to each expert's routing weight.
    """
    device = next(model.parameters()).device

    # Enable gradients only for gate weights
    for param in model.parameters():
        param.requires_grad_(False)
    gate_params = {}
    for layer_idx in moe_layer_indices:
    ...
    for batch in tqdm(calib_batches, desc="Collecting pure gradient scores (router)"):
        ...
        for layer_idx, gate_w in gate_params.items():
            if gate_w.grad is None:
                continue
            # Pure gradient magnitude: mean(|∂L/∂W|) per expert row
...
def collect_pure_expert_gradient_scores(model, calib_batches, moe_layer_indices):
    """
    Compute expert importance based on the average absolute gradient of each
    expert's own parameters (gate_proj, up_proj, down_proj).

    Score per expert = mean(|∂L/∂W|) averaged across all parameters.

    Unlike ``collect_expert_gradient_scores`` which uses the Taylor
    approximation |W * ∂L/∂W|, this measures pure gradient magnitude.
    """
    device = next(model.parameters()).device

    # Enable gradients only for routed-expert parameters
    for param in model.parameters():
        param.requires_grad_(False)

    expert_params = {}  # {layer_idx: [(eid, [param_list]), ...]}
    for layer_idx in moe_layer_indices:
        moe_block = get_moe_block(model.model.layers[layer_idx])
        if moe_block is None:
            continue
...
# ---------------------------------------------------------------------------
# Pure gradient magnitude score collection (dissected expert params)
# ---------------------------------------------------------------------------

def collect_pure_expert_gradient_scores_by_proj(model, calib_batches, moe_layer_indices, include_projs):
    """
    Variant of ``collect_pure_expert_gradient_scores`` that only counts gradients
    coming from a subset of an expert's projections.

    ``include_projs`` is an iterable subset of {"up_proj", "down_proj", "gate_proj"}.
    Score per expert = mean(|∂L/∂W|) averaged across the parameters of the
    selected projections only.
    """
    include_projs = set(include_projs)
    device = next(model.parameters()).device
    ...
    for layer_idx in moe_layer_indices:
        ...
        for eid, expert in enumerate(moe_block.experts):
            selected = []
            for proj_name in include_projs:
                proj = getattr(expert, proj_name, None)
                if proj is None:
                    raise AttributeError(
                        f"Expert at layer {layer_idx}, eid {eid} has no attribute '{proj_name}'"
                    )
                for p in proj.parameters():
                    p.requires_grad_(True)
                    selected.append(p)
            expert_params[layer_idx].append((eid, selected))
    ...
    desc = f"Collecting pure expert gradient scores ({'+'.join(sorted(include_projs))})"
    model.train()
...
# ---------------------------------------------------------------------------
# Expert-gradient-based score collection
# ---------------------------------------------------------------------------

def collect_expert_gradient_scores(model, calib_batches, moe_layer_indices):
    """
    Compute expert importance based on the gradient magnitude of each expert's
    own parameters (gate_proj, up_proj, down_proj).

    For each expert, the score is the accumulated L2 norm of (param * grad)
    across all its parameters and all calibration samples.  This is a Taylor
    first-order approximation of how much the loss would change if the expert
    were removed — unlike ``collect_gradient_scores`` which only looks at the
    router gate weights.
...
# ---------------------------------------------------------------------------
# DenseMixer gradient-based score collection
# ---------------------------------------------------------------------------

def _densemixer_qwen2_moe_forward(self, hidden_states: torch.Tensor):
    """
    DenseMixer forward for Qwen2MoeSparseMoeBlock (Qwen1.5-MoE / Qwen2-MoE).

    Forward value  = sparse (top-k) output
    Backward gradient = dense (all experts, full routing weights) output

    Achieved via straight-through estimator:
        final = sparse.detach() + (dense - dense.detach())
    """
    ...
    shared_expert_output = (
        F.sigmoid(self.shared_expert_gate(flat_hidden)) * shared_expert_output
    )
    ...
    for expert_idx in range(self.num_experts):
        ...
        # Gradient mask: expert params only receive grads from selected tokens
        activation_mask = (
    ...
    # Straight-through: forward = sparse, backward = dense
...
def collect_densemixer_gradient_scores(model, calib_batches, moe_layer_indices):
    """
    Compute expert importance via DenseMixer gradient on the router gate.

    Unlike standard ``gradient_pruning`` where gradients only flow through the
    top-k sparse path, DenseMixer uses a straight-through estimator so that
    **all** experts contribute to the backward gradient via a dense weighted sum.

    Formula per expert j:
        importance_j = mean( |∂L_dense/∂gate.W[j]| )

    where ∂L_dense/∂gate.W[j] is the gradient of loss w.r.t. the j-th row of
    the gate weight, computed through the dense MoE path (all experts weighted
    by full softmax routing probabilities).  Uses pure gradient magnitude
    (no Taylor weight multiplication), consistent with ``pure_gradient_pruning``.

    The forward value remains the standard sparse output; only the backward
    pass is modified via:
        final = sparse.detach() + (dense - dense.detach())
    """
    device = next(model.parameters()).device
    layers = model.model.layers

    # Patch MoE blocks with DenseMixer forward
    orig_forwards = {}
    for layer_idx in moe_layer_indices:
        moe_block = get_moe_block(layers[layer_idx])
        if moe_block is not None:
            ...
            moe_block.forward = types.MethodType(
                _densemixer_qwen2_moe_forward, moe_block
            )

    # Enable gradients only on gate weights
    for param in model.parameters():
        param.requires_grad_(False)
    gate_params = {}
    for layer_idx in moe_layer_indices:
...
def prune_model_inplace(model, keep_per_layer, moe_layer_indices):
    """
    Prune routed experts + gate in-place for each MoE layer.
    shared_expert and shared_expert_gate are NOT pruned.
    """
    layers = model.model.layers
    config = model.config
    num_layers = config.num_hidden_layers
    ...
    for layer_idx in moe_layer_indices:
        ...
        # 1. Prune experts ModuleList (only routed experts, not shared_expert)
...
def main():
    parser = argparse.ArgumentParser(description="Expert drop for Qwen1.5-MoE / Qwen2-MoE models")
    parser.add_argument("--model_name_or_path", type=str, required=True)
    parser.add_argument("--output_dir", type=str, required=True)
    parser.add_argument("--preserve_n", type=int, default=30,
                        help="Number of experts to preserve (per-layer for layerwise, average for global)")
    parser.add_argument("--method", type=str, default="global_pruning",
                        choices=["global_pruning", "layerwise_pruning", "bias_pruning",
                                 "gradient_pruning", "expert_gradient_pruning",
                                 "pure_gradient_pruning", "pure_expert_gradient_pruning",
    ...
    parser.add_argument("--random_seed", type=int, default=42,
                        help="[random_pruning] RNG seed for reproducible random expert selection")
    parser.add_argument("--n_samples", type=int, default=128)
    parser.add_argument("--seq_len", type=int, default=2048)
    parser.add_argument("--calib_data", type=str, default=None,
                        help="Path to local JSON calibration file (used when --dataset_name is not set)")
    parser.add_argument("--dataset_name", type=str, default=None,
                        help="HuggingFace dataset name (e.g. RoxanneWsyw/gsm)")
    ...
    parser.add_argument("--text_column", type=str, default="prompt",
                        help="Column(s) to use as calibration text. "
                             "Comma-separated to concatenate multiple columns (e.g. prompt,completion)")
    parser.add_argument("--calib_preset", type=str, default=None,
                        choices=["ceval", "math", "cmmlu"],
                        help="Benchmark preset (loads dev/train split of C-Eval / MATH / CMMLU). "
                             "Takes priority over --dataset_name and --calib_data.")
    parser.add_argument("--calib_preset_split", type=str, default=None,
                        help="Override the default split used by --calib_preset "
                             "(defaults: ceval=dev, cmmlu=dev, math=train).")
    parser.add_argument("--shuffle_seed", type=int, default=None,
                        help="Shuffle dataset with this seed before sampling (default: no shuffle)")
    # Bias pruning parameters
    parser.add_argument("--bias_update_speed", type=float, default=4e-2,
                        help="[bias_pruning] Gamma for bias update (default: 4e-2)")
    parser.add_argument("--bias_criterion", type=str, default="median",
                        choices=["mean", "median"],
                        help="[bias_pruning] Criterion for bias update (default: median)")
    parser.add_argument("--bias_clip", type=float, default=10.0,
                        help="[bias_pruning] Upper clip for bias (default: 10.0)")
    parser.add_argument("--bias_epochs", type=int, default=2,
                        help="[bias_pruning] Number of passes over calibration data (default: 2)")
    parser.add_argument("--bias_threshold", type=float, default=0.0,
                        help="[bias_pruning] Prune experts with bias < threshold (default: 0.0)")
    parser.add_argument("--dtype", type=str, default="bf16", choices=["bf16", "fp16", "fp32"])
    parser.add_argument("--mask_file", type=str, default=None,
                        help="Path to a pre-computed expert_drop_summary.json. "
                             "When set, calibration and scoring are skipped and the "
                             "per-layer kept experts from the file are applied directly.")
    ...
    if args.mask_file:
        ...
        print("Collecting gradient-based importance scores (gate weights) ...")
        scores_per_layer = collect_gradient_scores(model, calib_batches, moe_layer_indices)

        for lid in sorted(scores_per_layer.keys())[:3]:
            s = scores_per_layer[lid]
            print(f"  Layer {lid} gate gradient importance: min={s.min():.6f}, max={s.max():.6f}, mean={s.mean():.6f}")

        print(f"Pruning with method=gradient_pruning (layerwise), preserve_n={args.preserve_n} ...")
        keep_per_layer = decide_experts_to_keep(
            scores_per_layer, num_experts_per_layer, args.preserve_n, "layerwise_pruning"
        )

    elif args.method == "expert_gradient_pruning":
        print("Collecting expert-gradient-based importance scores (expert params) ...")
        scores_per_layer = collect_expert_gradient_scores(model, calib_batches, moe_layer_indices)

        for lid in sorted(scores_per_layer.keys())[:3]:
            s = scores_per_layer[lid]
            print(f"  Layer {lid} expert gradient importance: min={s.min():.6f}, max={s.max():.6f}, mean={s.mean():.6f}")

        print(f"Pruning with method=expert_gradient_pruning (layerwise), preserve_n={args.preserve_n} ...")
        keep_per_layer = decide_experts_to_keep(
            scores_per_layer, num_experts_per_layer, args.preserve_n, "layerwise_pruning"
        )
        ...
        include_projs = proj_map[args.method]
        print(f"Collecting pure expert gradient scores ({'+'.join(include_projs)}) ...")
        scores_per_layer = collect_pure_expert_gradient_scores_by_proj(
            model, calib_batches, moe_layer_indices, include_projs
        )

        for lid in sorted(scores_per_layer.keys())[:3]:
            s = scores_per_layer[lid]
            print(f"  Layer {lid} pure expert gradient ({'+'.join(include_projs)}): "
                  f"min={s.min():.6f}, max={s.max():.6f}, mean={s.mean():.6f}")

        print(f"Pruning with method={args.method} (layerwise), preserve_n={args.preserve_n} ...")
        keep_per_layer = decide_experts_to_keep(
            scores_per_layer, num_experts_per_layer, args.preserve_n, "layerwise_pruning"
        )

    elif args.method == "densemixer_gradient_pruning":
        print("Collecting DenseMixer gradient-based importance scores ...")
        scores_per_layer = collect_densemixer_gradient_scores(model, calib_batches, moe_layer_indices)

        for lid in sorted(scores_per_layer.keys())[:3]:
            s = scores_per_layer[lid]
            print(f"  Layer {lid} DenseMixer gradient importance: min={s.min():.6f}, max={s.max():.6f}, mean={s.mean():.6f}")

        print(f"Pruning with method=densemixer_gradient_pruning (layerwise), preserve_n={args.preserve_n} ...")
        keep_per_layer = decide_experts_to_keep(
            scores_per_layer, num_experts_per_layer, args.preserve_n, "layerwise_pruning"
        )
        ...
        # Use bias as a layerwise score and keep top-preserve_n. The original
        # threshold-based logic ignored preserve_n entirely and produced the
        # same model at every drop ratio.
        print(f"Pruning with method=bias_pruning (layerwise topk on bias), "
              f"preserve_n={args.preserve_n} ...")
        keep_per_layer = decide_experts_to_keep(
            bias_per_layer, num_experts_per_layer, args.preserve_n, "layerwise_pruning"
        )
    else:
