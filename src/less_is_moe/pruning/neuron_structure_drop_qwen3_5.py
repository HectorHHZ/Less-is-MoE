"""
Structural neuron-level drop for Qwen3.5-35B-A3B (and other Qwen3.5-MoE variants).
...
Importance metric (same as Qwen3 / Qwen1.5 ports):
...
Usage:
    scripts/prune/neuron_structure_drop_qwen3_5.sh \
...
# ---------------------------------------------------------------------------
# Qwen3.5-MoE arch compatibility helpers (same as expert_drop_qwen3_5.py)
# ---------------------------------------------------------------------------
...
def _load_qwen3_5_model(path, dtype):
    """Try AutoModelForCausalLM, fall back to VL / base auto classes."""
...
def collect_neuron_gradient_scores(model, calib_batches, moe_layer_indices):
    ...
    #   batched_specs[lid] = (experts_module, n_experts, d_ffn)   Qwen3.5/Qwen3 in transformers 5.x
    #   list_specs[lid]    = [(eid, gate_w, up_w, down_w)]        legacy ModuleList layout
    batched_specs = {}
    list_specs = {}
...
def find_zeroed_neurons(model, moe_layer_indices):
    """Scan each expert for neurons whose gate row is entirely zero.
    ...
    for layer_idx in moe_layer_indices:
        ...
        if gu is not None:
            # Batched: gu has shape (N, 2*d_ffn, hidden); gate part is rows [0..d_ffn)
            n_experts = experts.num_experts
            d_ffn = gu.shape[1] // 2
            zero_mask = (gu[:, :d_ffn, :].float().abs().sum(dim=2) == 0)  # (N, d_ffn)
            for eid in range(n_experts):
                drop_ids = torch.nonzero(zero_mask[eid], as_tuple=False).flatten().tolist()
...
def structurally_remove_neurons(model, drop_per_layer, moe_layer_indices):
    """
    Shrink each routed expert's gate/up/down projections to drop bottom-K neurons.
    All experts in a layer must drop the same COUNT (uniform surviving d_ffn) so
    the resulting weights stay rectangular; per-expert kept *indices* may differ.

    Supports both layouts:
      - Batched (Qwen3.5 / Qwen3 in transformers 5.x):
            experts.gate_up_proj  (N, 2*d_ffn, hidden)
            experts.down_proj     (N, hidden,  d_ffn)
        We use torch.gather along dim 1 / dim 2 with per-expert indices, then
        rewrap as nn.Parameter.
      - Legacy ModuleList: per-expert nn.Linear modules sliced via helpers.
    """
    layers = _get_decoder_layers(model)
    summary = []
    total_dropped = 0
    ...
    for layer_idx in moe_layer_indices:
        ...
        if gu is not None:
            ...
            n_experts = experts.num_experts
            orig_d_ffn = dp.shape[-1]
            hidden = gu.shape[-1]
            assert gu.shape[1] == 2 * orig_d_ffn, (
                f"Unexpected gate_up_proj shape {tuple(gu.shape)} vs d_ffn={orig_d_ffn}"
            )

            # Build per-expert keep_ids and verify uniform count.
            keep_per_expert = []
            n_kept_uniform = None
            for eid in range(n_experts):
                drop_ids = drop_per_layer.get(layer_idx, {}).get(eid, [])
                drop_set = set(drop_ids)
                keep_ids_list = [j for j in range(orig_d_ffn) if j not in drop_set]
                n_kept = len(keep_ids_list)
                if n_kept_uniform is None:
                    n_kept_uniform = n_kept
                elif n_kept_uniform != n_kept:
                    raise ValueError(
                        "Structural removal needs a uniform per-expert kept count "
                        f"in layer {layer_idx}; got {n_kept_uniform} and {n_kept} "
                        f"(expert {eid})."
                    )
                keep_per_expert.append(keep_ids_list)
            ...
            new_d_ffn = n_kept_uniform
            if new_d_ffn_seen is None:
                new_d_ffn_seen = new_d_ffn
            elif new_d_ffn_seen != new_d_ffn:
                raise ValueError(
                    f"Cross-layer d_ffn mismatch: {new_d_ffn_seen} vs {new_d_ffn} (layer {layer_idx})"
                )

            if new_d_ffn < orig_d_ffn:
                # Build (N, new_d_ffn) keep tensor on the right device.
                keep_t = torch.tensor(keep_per_expert, dtype=torch.long, device=gu.device)
                # gate_up_proj rows: gate at [0..orig_d_ffn), up at [orig_d_ffn..2*orig_d_ffn).
                gate_up_idx = torch.cat([keep_t, keep_t + orig_d_ffn], dim=1)         # (N, 2*new_d_ffn)
                gate_up_idx = gate_up_idx.unsqueeze(-1).expand(-1, -1, hidden)        # (N, 2*new_d_ffn, hidden)
                new_gu = torch.gather(gu.data, dim=1, index=gate_up_idx).contiguous()
                experts.gate_up_proj = nn.Parameter(new_gu.clone())

                down_idx = keep_t.unsqueeze(1).expand(-1, hidden, -1)                  # (N, hidden, new_d_ffn)
                new_dp = torch.gather(dp.data, dim=2, index=down_idx).contiguous()
                experts.down_proj = nn.Parameter(new_dp.clone())

            # Update inner-attribute the experts module reads at forward time.
            if hasattr(experts, "intermediate_dim"):
                experts.intermediate_dim = new_d_ffn
            if hasattr(experts, "intermediate_size"):
                experts.intermediate_size = new_d_ffn

            avg_drop = (sum(len(drop_per_layer.get(layer_idx, {}).get(e, []))
                            for e in range(n_experts)) / max(n_experts, 1))
            print(f"  Layer {layer_idx}: {n_experts} experts (batched), "
                  f"avg {avg_drop:.0f} neurons removed -> d_ffn={new_d_ffn}")
        else:
            # ---------- Legacy ModuleList layout ----------
            for eid, expert in enumerate(experts):
                ...
                if drop_ids:
                    keep_ids = torch.tensor(
                        keep_ids_list, dtype=torch.long,
            ...
            avg_drop = (sum(len(drop_per_layer.get(layer_idx, {}).get(e, []))
                            for e in range(n_experts)) / max(n_experts, 1))
            print(f"  Layer {layer_idx}: {n_experts} experts (ModuleList), "
...
def main():
    parser = argparse.ArgumentParser(
        description="Structural neuron-level drop for Qwen3.5-MoE"
    )
    ...
    print(f"Loaded {type(model).__name__} "
          f"(num_experts={getattr(_get_text_config(model.config), 'num_experts', '?')}, "
    ...
    first_moe = get_moe_block(_get_decoder_layers(model)[moe_layer_indices[0]])
    # d_ffn lives in batched gate_up_proj.shape[1]/2 (Qwen3.5 / Qwen3 in 5.x)
    # or in per-expert gate_proj.weight.shape[0] (legacy ModuleList).
    if hasattr(first_moe.experts, "gate_up_proj"):
        orig_d_ffn = first_moe.experts.gate_up_proj.shape[1] // 2
    else:
    ...
    # Qwen3.5-MoE stores moe_intermediate_size under text_config (nested) for
    # VL checkpoints; pure-text Qwen3/3.5 keeps it on model.config. Write to
    # whichever is resolved by _get_text_config.
    tc = _get_text_config(model.config)
    old_moe_int = getattr(tc, "moe_intermediate_size", None)
    tc.moe_intermediate_size = new_d_ffn
