...
class GPT4Evaluator(BaseEvaluator):
    def __init__(self, dataset, config):
        ...
        try:
            ...
            raise RuntimeError(
                "The ESFT LLM judge requires the optional 'judge' extra: "
                "pip install -e '.[judge]'"
            ) from exc
...
class IntentEvaluator(BaseEvaluator):
    ...
    def post_process(self, raw_prediction, ground_truths):
        ...
        # Fix missing leading '{' — common artifact from pruned MoE models where
        # the tokenizer merges ' {"' into a single token (id=5212) but the pruned
        # model generates ' "' (id=330) instead, dropping the opening brace.
        if pred and not pred.startswith('{'):
            if pred.startswith('"intent"') or pred.startswith(' "intent"'):
                pred = '{' + pred.lstrip()
