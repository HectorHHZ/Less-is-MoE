...
def push_to_hub_repo(
    training_args: SFTConfig | GRPOConfig,
    commit_message: str,
    extra_ignore_patterns=None,
) -> Future:
    """Pushes the model to a Hub repo."""

    private = True
    if getattr(training_args, "hub_private_repo", None) is not None:
        private = training_args.hub_private_repo

    repo_url = create_repo(repo_id=training_args.hub_model_id, private=private, exist_ok=True)
    logger.info(f"Created target repo at {repo_url}")
    ignore_patterns = ["*.pth"]
    ...
    logger.info(f"Pushed to {repo_url} successfully!")
